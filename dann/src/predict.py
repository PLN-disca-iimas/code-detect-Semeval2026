...
def load_model_and_tokenizer(train_path, artifact_name, device):
...
@torch.no_grad()
def predict(train_path, test_path, artifact_name, output_path, max_length, batch_size=16, device=None):
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"

    # Load model & tokenizer
    model, tokenizer = load_model_and_tokenizer(train_path, artifact_name, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--artifact_name", type=str, required=True, help="Wandb artifact name containing the trained model weights")
