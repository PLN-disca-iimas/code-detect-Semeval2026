...
class CodeBERTTrainer:
    ...
    def load_and_prepare_data(self, train_path, val_path):
        ...
        try:
            ...
            logger.info(f"Label range: {train_df['label'].min()} to {train_df['label'].max()}")
    ...
    def prepare_datasets(self, train_df, val_df, not_prepared = True, Task_B = 0):
    ...
    def train(self, run_name, train_dataset, val_dataset, num_epochs=3, batch_size=32, learning_rate=1e-5, lambda_domain = 0.5, gradient_accumulation = 2, eval_step = 800, training_step = 80):
    ...
    def evaluate_model(self, dataloader, device, show_classification_report = False):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                all_label_loss = all_label_loss + label_loss.item()
    ...
    def run_full_pipeline(self, train_path, val_path, run_name, num_epochs=3, batch_size=16, learning_rate=2e-5, not_prepared = True, Task_B = 0):
        try:
            train_df, val_df = self.load_and_prepare_data(train_path, val_path)
            
            self.initialize_model_and_tokenizer()
            
            train_dataset, val_dataset = self.prepare_datasets(train_df, val_df, not_prepared, Task_B)
